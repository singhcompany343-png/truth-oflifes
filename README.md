...
## Reference UI + stability fixes (Sep 2026)
- Updated the landing page to match the supplied Truth.ofLifes reference: pale blue hero, nurse/white-kitten reference artwork, four feature tiles, compact navigation, and a working mobile hamburger menu.
- Added an explicit, safe route for the mascot artwork and explicit page routes for the bundled reels, quiz, and student pages. The project directory is not exposed with a blanket static-file middleware.
- Added declared Node dependencies required by the server and seed scripts so the Render build can install them.
- Fixed client-side quiz timeout behavior to submit an unanswered item and automatically proceed after the server accepts it. The server remains authoritative for timing and scoring.
- The JSON bank contains 2,000 items (100 per each of 20 subject slugs); they are practice variants and are not represented as 2,000 independently authored or educator-reviewed questions. Review clinically before formal use.

## Deployment safety
This archive does not modify the live Render service or production database. Back up the database and uploaded PDFs before applying the additive learning migration. Set DATABASE_URL, a unique strong JWT_SECRET, ADMIN_USERNAME, and ADMIN_PASSWORD in Render. Deploy to a preview/test service and smoke-test login, download, quiz timeout, scoring, certificates, and admin actions before switching production traffic.
